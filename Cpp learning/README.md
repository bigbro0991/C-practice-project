...
![image](https://github.com/bigbro0991/CPP-practice-project/blob/master/Cpp%20learning/image002.png)
...
父类中私有成员属性 是被编译器给隐藏了，因此是访问不到的，但确实是被继承下去了。
...
2.子类要重写父类的虚函数（返回类型 函数名 参数列表 完全相同 virtual 关键字在子函数可写可不写）

动态多态的使用:父类的指针或者引用指向子类对象

子类要重写父类的虚函数 重写：（返回类型 函数名 参数列表 完全相同 virtual 关键字在子函数可写可不写）



virtual 后 sizeof 父类 大小为4 创建了一个指针vfptr

vfpter 指向 vftable， vftable 记录 函数的地址 &Animal::speak



> 10tenth/muti_state.cpp
>
> 多态满足条件
>
> - 有继承关系
...
**工程化的时候，必须new 派生类 用 base类 一个*ptr 来接收这个new出的类的指针，直接派生类指针 不行。**